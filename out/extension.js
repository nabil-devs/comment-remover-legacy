...
class CommentRemover {
    ...
    async removeAllComments() {
        ...
        const choice = await vscode.window.showWarningMessage('Remove comments from entire workspace?', { modal: true, detail: 'This will process all supported file types.' }, 'Preview Changes', 'Remove Comments', 'Cancel');
        if (choice === 'Cancel') {
            return;
        }
    ...
    async previewChanges() {
        ...
        if (stats.filesModified > 0) {
            const choice = await vscode.window.showInformationMessage(`Found ${stats.commentsRemoved} comments in ${stats.filesModified} files`, 'Apply Changes', 'Show Details', 'Cancel');
                ...
                for (let i = 0; i < extensions.length; i += batchSize) {
                    ...
                    try {
                        ...
                        for (const file of files) {
                            ...
                            const fileStats = await this.processFile(file.fsPath, removeSingleLine, removeMultiLine, applyChanges);
