...
class CommentRemover {
    ...
    async cleanupOldBackups() {
        ...
        const backupEnabled = config.get('backup.enabled', true);
        if (!backupEnabled)
            return;
        ...
        for (const folder of workspaceFolders) {
            const backupLocation = config.get('backup.location', '${workspaceFolder}/.comment-remover-backups')
    ...
    async createBackup(files) {
        const config = vscode.workspace.getConfiguration('commentRemoverPro');
        const backupEnabled = config.get('backup.enabled', true);
        if (!backupEnabled || files.length === 0)
            return null;
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders)
            return null;
    ...
    async removeAllComments() {
        ...
        const choice = await vscode.window.showWarningMessage('Remove comments from entire workspace?', {
            modal: true,
            detail: 'This will process all supported file types. A backup will be created automatically.'
        }, 'Preview Changes', 'Remove Comments', 'Cancel');
        if (choice === 'Cancel') {
            return;
        }
        ...
        if (result.stats.filesModified > 0) {
            ...
            vscode.window.showInformationMessage(`${message}\n${backupInfo}`, 'Show Details', 'Undo Changes', 'Star Repo').then(selection => {
    ...
    async previewChanges() {
        ...
        if (result.stats.filesModified > 0) {
            const choice = await vscode.window.showInformationMessage(`Found ${result.stats.commentsRemoved} comments in ${result.stats.filesModified} files`, 'Apply Changes', 'Show Details', 'Cancel');
    ...
    async undoLastRemoval() {
        ...
        const choice = await vscode.window.showWarningMessage('Restore from backup?', { modal: true, detail: 'This will restore all files modified in the last operation.' }, 'Restore', 'Cancel');
        if (choice !== 'Restore') {
            return;
        }
    ...
    async processWorkspace(applyChanges) {
        return vscode.window.withProgress({
            ...
            const removeSingleLine = config.get('remove.singleLine', true);
            const removeMultiLine = config.get('remove.multiLine', true);
            const workspaceFolders = vscode.workspace.workspaceFolders || [];
            ...
            for (let i = 0; i < filesToProcess.length; i++) {
                ...
                const fileStats = await this.processFile(filePath, removeSingleLine, removeMultiLine, applyChanges);
    ...
    async processFile(filePath, removeSingleLine, removeMultiLine, applyChanges) {
        try {
            const content = fs.readFileSync(filePath, 'utf8');
            const extension = path.extname(filePath).toLowerCase();
            const newContent = this.removeCommentsFromContent(content, extension, removeSingleLine, removeMultiLine);
    ...
    removeCommentsFromContent(content, extension, removeSingleLine, removeMultiLine) {
    ...
    countCommentDifferences(original, modified) {
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('commentRemoverPro.removeAllComments', () => {
